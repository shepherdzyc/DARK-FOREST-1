...
public class AccountManager
{
    ...
    private static async Task<bool> RequestLeaderboard(int uid)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            AccountManager.Instance.Menu.ShowPopup("无法获取排行榜", $"{response.StatusCode}: {errorMessage}");
    ...
    private static async Task<bool> SubmitScoreAsync(object data)
    {
        ...
        {
            var errorMessage = await response.Content.ReadAsStringAsync();
            AccountManager.Instance.Menu.ShowPopup("无法提交分数", $"{response.StatusCode}: {errorMessage}");
        ...
        AccountManager.Instance.Menu.ShowPopup("信息", $"分数上传成功!\n新的排名: #{result.rank}");
        return true;
    }
    ...
    private static async Task<bool> CreateAccountAsync(object data)
    {
        ...
        {
            var errorMessage = await response.Content.ReadAsStringAsync();
            AccountManager.Instance.Menu.ShowPopup("无法创建账号", $"{response.StatusCode}: {errorMessage}");

            return false;
        }

        var responseString = await response.Content.ReadAsStringAsync();
        AccountManager.Instance.Menu.ShowPopup("信息", $"创建账号成功");
        return true;
    }
    ...
    private static async Task<bool> LoginAsync(object data)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    var errorMessage = await response.Content.ReadAsStringAsync();
                    AccountManager.Instance.Menu.ShowPopup("无法登录", $"{response.StatusCode}: {errorMessage}");
